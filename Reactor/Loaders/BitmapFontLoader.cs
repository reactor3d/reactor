  ...
  public static class BitmapFontLoader
    {
        ...
        public static BitmapFont Load(Stream stream)
        {
            ...
            if (!stream.CanSeek)
                throw new ArgumentException("Stream must be seekable in order to determine file format.",
                    nameof(stream));

            switch (GetFileFormat(stream))
            {
        ...
        public static BitmapFont LoadBinary(Stream stream)
        {
            ...
            if (buffer[3] != 3) throw new InvalidDataException("Only BMFont version 3 format data is supported.");
            ...
            while (stream.Read(buffer, 0, 5) != 0)
            {
                ...
                if (blockSize > buffer.Length) buffer = new byte[blockSize];
        ...
        {
            return int.TryParse(GetNamedString(parts, name, estimatedStart), NumberStyles.Number,
        ...
        internal static string GetNamedString(string[] parts, string name, int estimatedStart)
        {
            ...
            if (string.Equals(GetValueName(parts[estimatedStart]), name, StringComparison.OrdinalIgnoreCase))
            {
                ...
                result = SanitizeValue(parts[estimatedStart].Substring(name.Length + 1));
            }
            else
            {
                ...
                for (var i = 0; i < parts.Length; i++)
                {
                    ...
                    {
                        result = SanitizeValue(part.Substring(name.Length + 1));
                        break;
                    }
                }
            }
        ...
        public static BitmapFont LoadText(TextReader reader)
        {
            ...
            do
            {
                ...
                if (line != null)
                {
                    ...
                    if (parts.Length != 0)
                        ...
                        {
                            case "info":
                                f.FamilyName = GetNamedString(parts, "face", 1);
                                f.FontSize = GetNamedInt(parts, "size", 2);
                                f.Bold = GetNamedBool(parts, "bold", 3);
                                f.Italic = GetNamedBool(parts, "italic", 4);
                                f.Charset = GetNamedString(parts, "charset", 5);
                                f.Unicode = GetNamedBool(parts, "unicode", 6);
                                f.StretchedHeight = GetNamedInt(parts, "stretchH", 7);
                                f.Smoothed = GetNamedBool(parts, "smooth", 8);
                                f.SuperSampling = GetNamedInt(parts, "aa", 9);
                                f.Padding = ParsePadding(GetNamedString(parts, "padding", 10));
                                f.Spacing = ParsePoint(GetNamedString(parts, "spacing", 11));
                                f.OutlineSize = GetNamedInt(parts, "outline", 12);
                                break;

                            case "common":
                                f.LineHeight = GetNamedInt(parts, "lineHeight", 1);
                                f.BaseHeight = GetNamedInt(parts, "base", 2);
                                f.TextureSize = new Size(GetNamedInt(parts, "scaleW", 3),
                                    GetNamedInt(parts, "scaleH", 4));
                                // TODO: 5 is pages, which we currently don't directly read
                                f.Packed = GetNamedBool(parts, "packed", 6);
                                f.AlphaChannel = GetNamedInt(parts, "alphaChnl", 7);
                                f.RedChannel = GetNamedInt(parts, "redChnl", 8);
                                f.GreenChannel = GetNamedInt(parts, "greenChnl", 9);
                                f.BlueChannel = GetNamedInt(parts, "blueChnl", 10);
                                break;
                            ...
                            case "page":
                                ...
                                id = GetNamedInt(parts, "id", 1);
                                name = GetNamedString(parts, "file", 2);
                            ...
                            case "char":
                                ...
                                index = GetNamedInt(parts, "id", 1);

                                charData = new Character
                                (
                                    index >= 0 ? (char)index : '\0',
                                    GetNamedInt(parts, "x", 2),
                                    GetNamedInt(parts, "y", 3),
                                    GetNamedInt(parts, "width", 4),
                                    GetNamedInt(parts, "height", 5),
                                    GetNamedInt(parts, "xoffset", 6),
                                    GetNamedInt(parts, "yoffset", 7),
                                    GetNamedInt(parts, "xadvance", 8),
                                    GetNamedInt(parts, "page", 9),
                                    GetNamedInt(parts, "chnl", 10)
                                );

                                if (index == -1) f.InvalidChar = charData;
                            ...
                            case "kerning":
                                ...
                                key = new Kerning((char)GetNamedInt(parts, "first", 1),
                                    (char)GetNamedInt(parts, "second", 2),
                                    GetNamedInt(parts, "amount", 3));
            ...
            f.Pages = ToArray(pageData.Values);
        ...
        public static BitmapFont LoadXml(TextReader reader)
        {
            ...
            f.TextureSize = new Size(
                Convert.ToInt32(properties.Attributes["scaleW"].Value, CultureInfo.InvariantCulture),
            ...
            foreach (XmlNode node in root.SelectNodes("pages/page"))
            {
                ...
                page = new Page(Convert.ToInt32(node.Attributes["id"].Value, CultureInfo.InvariantCulture),
            ...
            foreach (XmlNode node in root.SelectNodes("chars/char"))
            {
                ...
                if (index == -1) f.InvalidChar = character;
        ...
        /// <param name="buffer">
        ///     The output buffer where split strings will be placed. Must be larged enough to handle the contents
        ///     of <paramref name="s" />.
        /// </param>
        /// <returns></returns>
        internal static void Split(string s, string[] buffer)
        {
            ...
            for (; index < buffer.Length; index++) buffer[index] = string.Empty;
        }
        ...
        private static void LoadCharactersBlock(ref BitmapFont font, byte[] buffer, int blockSize)
        {
            ...
            charCount = blockSize /
                        20; // The number of characters in the file can be computed by taking the size of the block and dividing with the size of the charInfo structure, i.e.: numChars = charsBlock.blockSize/20.
            ...
            for (var i = 0; i < charCount; i++)
            {
                ...
                if (index == -1) font.InvalidChar = chr;
        ...
        private static void LoadCommonBlock(ref BitmapFont font, byte[] buffer)
        {
            ...
            font.TextureSize = new Size(WordHelpers.MakeWordLittleEndian(buffer, 4),
                WordHelpers.MakeWordLittleEndian(buffer, 6));
