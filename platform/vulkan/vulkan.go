...
import (
	...
	assets "github.com/reactor3d/reactor/assets"
...
func (v VulkanDeviceInfo) CreateBuffers(vertexData ArrayFloat32) (VulkanBufferInfo, error) {
	gpu := v.gpuDevices[0]
...
func LoadShader(device vk.Device, name string, src []byte) (vk.ShaderModule, error) {
...
func CreateGraphicsPipeline(device vk.Device,
	...
	vdata, _ := assets.FS.ReadFile("shaders/tri-vert.glsl")
	fdata, _ := assets.FS.ReadFile("shaders/tri-frag.glsl")
	// Phase 2: load shaders and specify shader stages
	vertexShader, err := LoadShader(device, "shaders/tri-vert.spv", vdata)
	if err != nil { // err has enough info
		return gfxPipeline, err
	}
	defer vk.DestroyShaderModule(device, vertexShader, nil)

	fragmentShader, err := LoadShader(device, "shaders/tri-frag.spv", fdata)
	if err != nil { // err has enough info
		return gfxPipeline, err
	}
...
func VkInit(serverMode bool) error {
	if serverMode {
